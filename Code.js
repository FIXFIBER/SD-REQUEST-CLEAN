...
function _rejectionTarget(status, route) {
  if (route === 'metro') {
    ...
    if (status === 'Awaiting Planning Team') return 'Awaiting Service Delivery (Metro)';
    if (status === 'Awaiting Service Delivery (Metro)') return 'Rejected';
    if (status === 'Awaiting Final MEC Review') return 'Awaiting Service Delivery (Metro)';
...
function slaFreezeWindowFor(t) {
  ...
  if (day === 5 && (h > 23 || (h === 23 && m >= 59))) {
    var start = new Date(d); start.setHours(23, 59, 0, 0);
    var end = new Date(d); end.setDate(d.getDate() + 3); end.setHours(9, 30, 0, 0);
    return { start: start.getTime(), end: end.getTime() };
  }
  // Saturday or Sunday -> freeze until Monday 09:30:00
  if (day === 6 || day === 0) {
    var st = new Date(d); st.setHours(0, 0, 0, 0);
    var en = new Date(d);
    // roll to Monday
    var add = (day === 6) ? 2 : 1;
    en.setDate(d.getDate() + add); en.setHours(9, 30, 0, 0);
...
function slaFreezeInRange(a, b) {
  ...
  var total = 0;
  var guard = 0, MAX_FREEZE_ITERS = 366; // hard stop so a bad timestamp can't hang the run
  ...
  if (!win) {
    ...
    var nxt = new Date(d); nxt.setDate(d.getDate() + daysUntilFri); nxt.setHours(23, 59, 0, 0);
...
function getSLAAnalytics(token) {
  try {
    ...
    for (var i = 1; i < data.length; i++) {
      ...
      var elapsedPercent = Math.min(100, Math.max(0, Math.round((SLA_DURATION_MS - remainingForPct) / SLA_DURATION_MS * 100)));
...
function getPerformanceScores(token, force) {
  ...
  try {
    ...
    } catch (ce) { /* cache miss / corrupt — recompute below */ }

    var sh = _patSheet();
    if (!sh || sh.getLastRow() < 2) {
      return { success: true, officers: [], generatedAt: new Date().toISOString() };
    }
    ...
    try {
      ...
      var maxCols = (typeof sh.getMaxColumns === 'function') ? sh.getMaxColumns() : 37;
    ...
    for (var i = 1; i < data.length; i++) {
      ...
      try {
        ...
        if (!map[officer]) {
          map[officer] = {
            officer: officer,
            department: String(r[c.SUBMITTED_BY_DEPT] || r[c.ASSIGNED_TO_DEPT] || r[c.DEPARTMENT] || 'Unassigned').trim(),
            patCount: 0, snagSum: 0,
            notBreached: 0, totalForSla: 0,
        ...
        var g = map[officer];
        g.patCount++;
        var verdict = String(r[c.VERDICT] || '').trim().toLowerCase();
        ...
        var pass2 = (verdict === 'approved') ? 100 : (verdict === 'rejected' ? 0 : 100);
        var rowScore = Math.round(q2 * 0.40 + sla2 * 0.35 + pass2 * 0.25);
        ...
        if (!isNaN(ts)) { g.timestamps.push(ts); g.windowScores.push(rowScore); }
        g.pats.push({
    ...
    try {
      var _usheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(SD.USERS);
        ...
        for (var _ui = 1; _ui < _udata.length; _ui++) {
          ...
          if (!map[_key]) {
            map[_key] = {
              ...
              notBreached: 0, totalForSla: 0,
              approved: 0, rejected: 0, other: 0,
              pats: [], timestamps: [], windowScores: []
            };
          }
        }
      }
    } catch (_seedErr) { /* best-effort: board still works with PAT-derived officers */ }
    ...
    var officers = Object.keys(map).map(function(name) {
      ...
      if (g.patCount === 0) {
        return {
          officer: g.officer, department: g.department,
          patCount: 0, avgSnagScore: 0,
          qualityScore: null, slaCompliance: null, passRate: null,
          approved: 0, rejected: 0, score: null,
          avgDay: null, avgWeek: null, avgMonth: null,
          pats: [], noData: true
        };
      }
      ...
      var slaPct = g.totalForSla ? Math.round(g.notBreached / g.totalForSla * 100) : 100;
      ...
      var raw = quality * 0.40 + slaPct * 0.35 + passRate * 0.25;
      ...
      function _avgInWindow(ms) {
        ...
        for (var i = 0; i < g.timestamps.length; i++) {
          if (now - g.timestamps[i] <= ms) { sum += g.windowScores[i]; n++; }
        }
    ...
    officers.sort(function(a, b) {
      ...
      var aNull = a.score === null, bNull = b.score === null;
    ...
    officers.forEach(function(o, idx) { o.rank = idx + 1; });

    var result = { success: true, officers: officers, generatedAt: new Date().toISOString() };
...
function _invalidatePerfCache() {
  try { CacheService.getScriptCache().remove("perf_scores_v1"); } catch (e) {}
}
...
function assignPresidingOfficer(token, projectId, officerName) {
  try {
    ...
    if (!isMEC && !isSuperAdmin) {
      throw new Error('Only MEC personnel or a Super Admin may assign a Presiding Officer.');
    }
    if (!projectId || !officerName) throw new Error('Project ID and officer name are required.');
    var sh = _patSheet();
    var data = sh.getDataRange().getValues();
    var c = _patCols();
    ...
    var target = -1;
    for (var i = 1; i < data.length; i++) {
      var rid = String(data[i][c.PROJECT_ID] || '').trim().toUpperCase();
      if (rid === norm || ('PAT-' + rid) === norm || ('FOB-' + rid) === norm) { target = i; break; }
    }
    ...
    try {
      ...
      for (var j = 1; j < ud.length; j++) {
        var uname = String(ud[j][1] || '').trim().toLowerCase();
        var uemail = String(ud[j][2] || '').trim().toLowerCase();
        if (uname === String(officerName).trim().toLowerCase() || uemail === String(officerName).trim().toLowerCase()) {
          dept = String(ud[j][6] || ''); break;
        }
      }
    } catch(e2) {}

    sh.getRange(target + 1, c.PRESIDING_OFFICER + 1).setValue(String(officerName).trim());
    _invalidatePerfCache();
    return { success: true, message: 'Presiding Officer updated.', officer: String(officerName).trim(), department: dept };
  } catch(e) {
    return { success: false, message: e.message };
  }
}
...
function getPATProjectById(token, projectId) {
  try {
    ...
    var hasRoute = hist.some(function (h) { return h && h.entryRoute; });
    var isLegacyMetro = (foundProject.workflowStatus === 'Awaiting Service Delivery') && !hasRoute;
    if (isLegacyMetro) {
      ...
      try { savePATProject(null, foundProject); } catch (e) { /* read-only-safe */ }
    }
...
function generateVendorReviewLink(token, projectId) {
  try {
    ...
    if (!isProjectTeam && !isSDMetro) {
      return { success: false, message: 'Only Project Team or Service Delivery / Metro members can generate vendor review links.' };
    }
    ...
    var canVendor = (project.workflowStatus === 'Awaiting Project Team') ||
                    (project.workflowStatus === 'Awaiting Service Delivery (Metro)');
    if (!canVendor) {
      return { success: false, message: 'Vendor review is only available at the Awaiting Project Team or Awaiting Service Delivery (Metro) stage.' };
    }
 ...
 */
function submitPATToDepartment(token, projectId, targetDept, comments, entryRoute) {
  try {
    var sess = _session(token);
    ...
    var explicitRoute = (entryRoute === 'metro' || entryRoute === 'project') ? entryRoute
                      : _entryRouteOf(project);
    if (project.workflowStatus === 'Draft' || project.workflowStatus === 'Rejected') {
      if (targetDeptNorm.indexOf('metro') !== -1 || targetDeptNorm.indexOf('service delivery') !== -1) {
    ...
    if (project.workflowStatus === 'Awaiting Final MEC Review' && isRejection) {
      ...
      newStatus = (fr === 'metro') ? 'Awaiting Service Delivery (Metro)' : 'Awaiting Project Team';
    ...
    if ((targetDeptNorm === 'mec' || targetDeptNorm === 'mech') &&
        (project.workflowStatus === 'Awaiting Project Team' || project.workflowStatus === 'Awaiting Service Delivery (Metro)' || project.workflowStatus === 'Rejected')) {
      if (isRejection) {
...
function submitPATToNextStage(token, projectId, nextEmail, comments) {
  try {
    ...
    var newStatus = _wfStageFlow(_entryRouteOf(project))[project.workflowStatus] || null;
...
function partiallyApprovePAT(token, projectId, comments, newSnags, nextDept, nextEmail) {
  try {
    ...
    var newStatus = _wfStageFlow(_entryRouteOf(project))[project.workflowStatus] || 'Partially Approved';
 ...
 */
function routePAT(token, projectId, action, comments, recipientEmail, entryRoute) {
  try {
    var sess = _session(token);
    var res = getPATProjectById(null, projectId);
    if (!res.success) return res;
    ...
    // ── MEC reject from Final Review → back to entry department ──
      ...
      newStatus = (route === 'metro') ? 'Awaiting Service Delivery (Metro)' : 'Awaiting Project Team';
      project.verdict = 'Rejected';
    }
    // ── MEC recheck handling (pushing to MEC from entry stage) ──
    else if ((current === 'Awaiting Project Team' || current === 'Awaiting Service Delivery (Metro)') && route === 'metro' && _normalizeDept(sess.department) === 'mec' && !isReject) {
      newStatus = (project.verdict === 'Rejected') ? 'Awaiting Final MEC Review' : 'Awaiting MEC Recheck';
    }
    else if ((current === 'Awaiting Project Team' || current === 'Awaiting Service Delivery (Metro)') && _normalizeDept(sess.department) === 'mec' && !isReject) {
      newStatus = (project.verdict === 'Rejected') ? 'Awaiting Final MEC Review' : 'Awaiting MEC Recheck';
    }
    // ── Rejections at any stage → route-aware rejection target ──
    else if (isReject) {
      newStatus = _rejectionTarget(current, route);
      if (current === 'Awaiting Service Delivery (Metro)' || (route === 'metro' && current === 'Awaiting Service Delivery')) {
    ...
    if ((current === 'Draft' || current === 'Rejected') && route) historyEntry.entryRoute = route;
    history.push(historyEntry);
    ...
    if (act === 'partial') {
      ...
      if (vCheck.required && !vCheck.approved) {
        return { success: false, message: vCheck.status === 'rejected'
          ? 'Vendor rejected this project. Please resolve with vendor before proceeding.'
          : 'Vendor approval is required before proceeding. Please wait for vendor response.' };
      }
    }
    ...
    if (recipientEmail) {
      ...
      try {
        ...
        var userData = userSh.getDataRange().getValues();
        for (var i = 1; i < userData.length; i++) {
          if (String(userData[i][2]).toLowerCase() === recipientEmail.toLowerCase()) {
            project.assignedToName = userData[i][1];
            project.assignedToDept = userData[i][6];
            project.department = userData[i][6];
            break;
    ...
    var saveRes = savePATProject(token, project);
    var msg = isReject ? ('Rejected & returned. Status: ' + newStatus)
...
function addSinglePATImage(token, projectId, imageData) {
  try {
    ...
    // Otherwise, if it's still Base64, process it as a file in Drive
      ...
      try {
        _appendImageDocRow(token, projectId, url, file.getId(), fileName, mime, bytes.length);
      } catch (docErr) { console.warn('PAT single image doc-row: ' + docErr.message); }
    } else {
 ...
 */
function _appendImageDocRow(token, projectId, driveUrl, driveFileId, fileName, mime, fileSize) {
  try {
    ...
    const data = sh.getDataRange().getValues();
    for (let i = 1; i < data.length; i++) {
      if (String(data[i][c.DRIVE_FILE_ID] || '') === String(driveFileId)) return;
    }

    let sess = null;
    try { sess = _session(token); } catch (e) { sess = null; }
...
function handleRequest(e) {
  ...
  try {
    ...
    switch(action) {
      ...
      case "routePAT": result = routePAT.apply(null, args); break;
      ...
      case "getPerformanceScores": result = getPerformanceScores.apply(null, args); break;
      case "assignPresidingOfficer": result = assignPresidingOfficer.apply(null, args); break;
...
function _createMaterial(description) {
  try {
    ...
    for (var i = 1; i < existing.length; i++) {
      if (String(existing[i][1] || '').trim().toLowerCase() === desc.toLowerCase()) {
        return { success: true, material: { id: existing[i][0], description: existing[i][1] }, isNew: false, message: 'Material already exists.' };
      }
    }
    var id = _genId("MAT-");
    sh.appendRow([id, desc, new Date().toISOString()]);
    return { success: true, material: { id: id, description: desc }, isNew: true, message: 'Material added.' };
  } catch(e) {
    return { success: false, message: e.message };
  }
}
